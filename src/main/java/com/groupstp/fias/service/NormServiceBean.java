...
public class NormServiceBean implements NormService {
    ...
    public Address normalize(String srcAddress) {
        ...
        if (parents.size() > 0) {
            ...
            parents.forEach(entity -> {
                if (entity.getClass().equals(House.class)) houses.add((House) entity);
            });
            address.setHouse(houses);

            final List<Stead> steads = new ArrayList<>();
            parents.forEach(entity -> {
                if (entity.getClass().equals(Stead.class)) steads.add((Stead) entity);
            });
            address.setStead(steads);

            final List<FiasEntity> entities = new ArrayList<>();
            parents.forEach(entity -> {
                if (entity instanceof FiasEntity) entities.add((FiasEntity) entity);
            });
    ...
    private Address normalizeInternal(String address, boolean force) {
        ...
        if (force || (a != null && a.__getEntityEntry().isNew()))
            a = normalize(address);
        return a;
    }
...
//        }
    ...
    private String constructAddress(List<StandardEntity> entities, String[] components, Integer lastComponent) {
        ...
        entities.forEach(e -> {
            ...
            do {
                Long garId = eAdm.__getEntityEntry().getAttributeValue("garId");
                ...
                if (eAdm.getClass().equals(House.class)) {
                    ...
                    addrAdm.add(0, eAdm.__getEntityEntry().getAttributeValue("shortname") + " " + eAdm.__getEntityEntry().getAttributeValue("offname"));
                }
                eAdm = eAdm.__getEntityEntry().getAttributeValue("parentAdm");
            } while (eAdm != null);
            do {
                Long garId = eMun.__getEntityEntry().getAttributeValue("garId");
                ...
                if (eMun.getClass().equals(House.class)) {
                    ...
                    addrMun.add(0, eMun.__getEntityEntry().getAttributeValue("shortname") + " " + eMun.__getEntityEntry().getAttributeValue("offname"));
                }
                eMun = eMun.__getEntityEntry().getAttributeValue("parentMun");
            } while (eMun != null);
